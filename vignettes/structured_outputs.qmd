...
## Using `generate_text` with `response_format`

The primary way to get structured data is to pass a `z_schema` to the `response_format` argument of `generate_text()`.
...
When you provide a `response_format`, **aisdk** intelligently determines the best way to interact with the LLM API:
