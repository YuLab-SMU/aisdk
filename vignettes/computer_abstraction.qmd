...
Atomic tools
  ...
  v
Computer layer
  - Scripts      # Unlimited actions via bash/filesystem
  - CLIs
  - Utilities
```
...
`Computer$execute_r_code()` always reports `execution_mode = "sandbox_exec"`.
This path uses an isolated `callr` process and does not mutate a live
`ChatSession$get_envir()`. If you need persistent session-scoped state across
turns or agents, use the session runtime (`ChatSession` / `SharedSession`)
instead of the computer abstraction.
